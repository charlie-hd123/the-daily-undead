...
test("browser-loaded code and styles share the current cache version", async () => {
  ...
  assert.deepEqual(new Set(versionTokens), new Set(["20260919-15"]));
});
...
test("account controls support optional sign-in without exposing private credentials", async () => {
  ...
  assert.match(account, /const currentUsername = accountButton\.textContent\.trim\(\)/);
  assert.match(account, /input\.defaultValue = currentUsername/);
  assert.match(
    account,
